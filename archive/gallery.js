(async function(){
  ...
  document.getElementById('count').textContent=man.count.toLocaleString()+' pieces, '+man.years[0]+' to '+man.years[man.years.length-1];
  ...
  function show(i){ if(i<0||i>=cur.length) return; idx=i; const it=cur[i]; lbimg.src=it.src; lbimg.style.width=it.w+'px'; lbimg.style.height=it.h+'px'; cap.textContent=man.brand+' · '+it.year+' · '+it.id+' · '+it.w+'×'+it.h+'px, shown at original size'; lb.classList.add('open'); }
